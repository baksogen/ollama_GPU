...
include(OllamaMoltenVKVersion)

function(_ollama_lookup_sha256 version out_var)
    set(_idx 0)
    list(LENGTH OLLAMA_MOLTENVK_RELEASE_SHA256_MAP _len)
    while(_idx LESS _len)
        math(EXPR _next "${_idx} + 1")
        list(GET OLLAMA_MOLTENVK_RELEASE_SHA256_MAP ${_idx} _ver)
        list(GET OLLAMA_MOLTENVK_RELEASE_SHA256_MAP ${_next} _sha)
        if(_ver STREQUAL version)
...
function(ollama_configure_moltenvk)
    ...
    set(_molten_sha256 "${OLLAMA_MOLTENVK_SHA256}")
    if (NOT _molten_url)
        ...
        if (NOT _molten_sha256)
            _ollama_lookup_sha256("${OLLAMA_MOLTENVK_VERSION}" _default_sha)
            if (_default_sha)
                set(_molten_sha256 "${_default_sha}")
            else()
                if (NOT OLLAMA_MOLTENVK_INSECURE)
                    message(FATAL_ERROR "OLLAMA_MOLTENVK_SHA256 must be provided for MoltenVK version ${OLLAMA_MOLTENVK_VERSION}, or set OLLAMA_MOLTENVK_INSECURE=ON to skip verification")
                endif()
            endif()
        endif()
    else()
        if (NOT _molten_sha256 AND NOT OLLAMA_MOLTENVK_INSECURE)
            message(FATAL_ERROR "OLLAMA_MOLTENVK_SHA256 must be set when OLLAMA_MOLTENVK_URL is overridden, or set OLLAMA_MOLTENVK_INSECURE=ON to skip verification")
        endif()
    endif()
    ...
    set(_requested_signature "${OLLAMA_MOLTENVK_VERSION}|${_molten_url}|${_molten_sha256}")
    ...
    if (_needs_download)
        ...
        if (_molten_sha256)
            list(APPEND _download_args EXPECTED_HASH "SHA256=${_molten_sha256}")
        endif()
